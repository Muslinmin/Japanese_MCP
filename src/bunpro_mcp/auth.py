...
#
# Keyed by code, valued by the moment the code expires: past that the
# signature no longer verifies, so remembering it protects nothing and
# would just grow forever on a long-lived instance.
_spent_codes: dict[str, float] = {}


def _mark_code_spent(code: str, expires_at: float) -> None:
    now = time.time()
    for spent, expiry in list(_spent_codes.items()):
        if expiry <= now:
            del _spent_codes[spent]
    _spent_codes[code] = expires_at
...
def _configured_token() -> str | None:
    """The deployment secret, with surrounding whitespace removed.

    Stripping is load-bearing, not tidiness. A secret generated by piping
    `openssl rand` into a secret store keeps the trailing newline, and a
    newline cannot be typed into the login form or sent in a header — so
    every login fails with "that token is not correct" and nothing
    indicates why. Tolerating it here costs nothing: no deliberate secret
    depends on its own leading or trailing whitespace.
    """
...
def verify(purpose: str, token: str | None) -> dict[str, Any] | None:
    ...
    try:
        ...
        # No secret configured: nothing can have been signed, so nothing
        # verifies. A misconfigured local run should 401, not 500.
        return None
...
class StatelessOAuthProvider:
    ...
    async def exchange_authorization_code(
        ...
        _mark_code_spent(authorization_code.code, authorization_code.expires_at)
        return self._issue(
            client_id=client.client_id,
